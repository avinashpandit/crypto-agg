...
func (e *Bw) UpdateAllBalances() {
	...
	if err := json.Unmarshal([]byte(jsonBalanceReturn), &jsonResponse); err != nil {
		...
		logger.Info().Msgf("%s UpdateAllBalances Failed: %v", e.GetName(), jsonResponse.ResMsg.Message)
		return
	}
	if err := json.Unmarshal(jsonResponse.Datas, &accountBalance); err != nil {
		logger.Info().Msgf("%s UpdateAllBalances Result Unmarshal Err: %v %s", e.GetName(), err, jsonResponse.Datas)
		return
	}
...
func (e *Bw) Withdraw(coin *coin.Coin, quantity float64, addr, tag string) bool {
	...
	if err := json.Unmarshal([]byte(jsonSubmitWithdraw), &jsonResponse); err != nil {
		...
		logger.Info().Msgf("%s Withdraw Failed: %v", e.GetName(), jsonResponse.ResMsg.Message)
		return false
	}
	if err := json.Unmarshal(jsonResponse.Datas, &withdraw); err != nil {
		logger.Info().Msgf("%s Withdraw Result Unmarshal Err: %v %s", e.GetName(), err, jsonResponse.Datas)
		return false
	}
